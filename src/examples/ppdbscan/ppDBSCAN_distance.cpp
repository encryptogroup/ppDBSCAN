...
int32_t read_test_options(int32_t* argcp, char*** argvp, e_role* role,
                          ...
                          uint16_t* port, int32_t* test_op, uint32_t* noLineSegments,std::string* dataset) {
...
int main(int argc, char** argv) {
  ...
  e_role role;
    ...
    read_test_options(&argc, &argv, &role, &bitlen, &nvals, &secparam, &address,
                      &port, &test_op, &noLineSegments, &dataset);

    seclvl seclvl = get_sec_lvl(secparam);

    testDistancesCalculation(role, address, port, seclvl, bitlen, nthreads, mt_alg,
                   euclidean_sharing, noLineSegments, dataset);
